...
```{r}
mpo_placebo_rolling_plot <-
    mpo_60s_rolling_only_pla %>% 
    ggplot()+
    stat_summary(aes(x = time,
                     y = po_rolling,
                     color = "mean"
    ...
    scale_color_manual(
            values = c("mean" = "black", "individual" = "gray"),
            labels = c("mean" = "Mean", "individual" = "Individual")
        ) +
    guides(color = guide_legend(title = NULL))+
    scale_x_continuous(breaks = seq(0, 360, 30))+
    labs(y = "PO (60 s rolling average, placebo trial; W)", x = "Time (s)")+
    facet_grid(~sex,
               labeller = as_labeller(c(W = "Females", M = "Males")))

ggsave(here::here('data/figures/tt/individual_60s_rolling_placebo_trial.svg'), height = 100, width = 200, units = "mm")
```
...
mpo_rolling_plot <-
    ...
    scale_x_continuous(breaks = seq(0, 360, 30))+
    labs(y = "PO (60 s rolling average vs. placebo; W)", x = "Time (s)", color = "Treatment")+
    facet_grid(~sex,
               labeller = as_labeller(c(W = "Females", M = "Males")))
...
Layout
```{r}
combined_plot <- mpo_placebo_rolling_plot / mpo_rolling_plot

ggsave(here::here('data/figures/tt/combined_plot.svg'), height = 200, width = 200, units = "mm")
```

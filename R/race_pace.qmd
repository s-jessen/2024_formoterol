...
mpo_30s_rolling <- mpo %>% 
    ...
    dplyr::mutate(po_delta_rolling = roll_mean(delta_po, n=30, fill="", align="right")) %>% 
    dplyr::mutate(po_delta_rolling_bw = roll_mean(delta_po_bw, n=30, fill="", align="right")) %>% 
    dplyr::mutate(po_delta_rolling_llm = roll_mean(delta_po_llm, n=30, fill="", align="right")) %>%
    dplyr::mutate(po_rolling = roll_mean(po, n=30, fill="", align="right")) %>% 
    dplyr::mutate(po_rolling_bw = roll_mean(po_bw, n=30, fill="", align="right")) %>% 
    dplyr::mutate(po_rolling_llm = roll_mean(po_llm, n=30, fill="", align="right")) %>%
    dplyr::ungroup() %>% 
...
mpo_30s <- mpo %>% 
    ...
    dplyr::ungroup() %>% 
    dplyr::group_by(id, treatment, ranges) %>% 
    dplyr::summarize(po = mean(po, na.rm = T),
                     po_llm = mean(po_llm, na.rm = T),
                     po_bw = mean(po_bw, na.rm = T)) %>% 
    dplyr::ungroup() %>% 
    dplyr::group_by(id, ranges) %>% 
    dplyr::mutate(delta_po = po - po[treatment == "pla"]) %>% 
    dplyr::mutate(delta_po_bw = po_bw - po_bw[treatment == "pla"]) %>% 
    dplyr::mutate(delta_po_llm = po_llm - po_llm[treatment == "pla"]) %>% 
    dplyr::ungroup() 
...
**30 s gennemsnit - som bar chart**
```{r, echo = F}
ggplot(mpo_30s)+
stat_summary(aes(x = ranges,
                     y = delta_po_llm,
                     fill = treatment
                     ),
    ...
    #scale_x_continuous(breaks = seq(0, 360, 30))+
    labs(y = "PO (average over 30 s intervals  vs. placebo)\n(W/kg llm)", x = "Interval", fill = "Treatment")
```
